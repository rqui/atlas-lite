...
mod tests {
    ...
    #[test]
    fn lookup_recovers_owned_tmp_groups_but_fails_closed_for_corrupt_group() {
        for (label, extension, retain_primary) in [
            ("primary-and-tmp", "TMP", true),
            ("tmp-only", "TMP", false),
            ("backup-only", "BAK", false),
        ] {
            let (repository, root) = repository(label, 4);
            ...
            let primary = fs::read_dir(&notes)
                ...
                .unwrap()
                .path();
            let temporary = primary.with_extension(extension);
            if retain_primary {
                fs::copy(&primary, &temporary).unwrap();
            } else {
                fs::rename(&primary, &temporary).unwrap();
            }
        ...
        let (repository, root) = repository("corrupt-recovery", 4);
        repository.store_note(note("missing"), metadata(1)).unwrap();
        let notes = root.join("CACHE/NOTES");
        let primary = fs::read_dir(&notes)
            .unwrap()
